...
- 報告 ID：`sdd-ai-robust-requirements-20260531-v11`
- 版本：`11.0.0`
- 產生時間：`2026-05-31T16:36:20Z`
...
- 本報告引用 `97` 筆來源，其中 `79` 筆為 A/B 層級，`47` 筆保存固定版本連結。
- [來源總目錄與各文件大綱](../sources/CATALOG.md)
- [完整機器可讀來源 metadata](sdd-ai-robust-requirements-20260531-v11.json)
- 個別來源的摘要與 metadata 請由來源總目錄進入；原始 snapshot 預設只保存在本機。

## 變更紀錄

- 11.0.0: GitHub 上傳策略改為只提交來源 metadata、hash、摘要與索引；raw snapshot 預設只保存在本機，必要時另存至有權限控管的 artifact storage。
- 10.0.0: Markdown 報告的來源章節改為精簡導航；完整來源 metadata 保留在同名 JSON 與 sources/CATALOG.md。新增研究補強判斷：現階段不需要無差別擴充文章，後續應優先執行 benchmark、brownfield PoC、安全政策核准與研究重現。
